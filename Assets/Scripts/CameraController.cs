...
public class CameraController : MonoBehaviour
{
    ...
    private void OnPointer(InputValue value)
    {
        ...
        //Debug.Log(moveDir);
        if (-padding <= mousePos.x && mousePos.x <= padding) moveDir.x = -1;
        else if(Screen.width - padding <= mousePos.x && mousePos.x <= Screen.width + padding) moveDir.x = 1;
        else moveDir.x = 0;

        if (-padding <= mousePos.y && mousePos.y <= padding) moveDir.y = -1;
        else if (Screen.height - padding <= mousePos.y && mousePos.y <= Screen.height + padding) moveDir.y = 1;
        else moveDir.y = 0;
    }
    private void Zoom()
    {
